function [Packet,Record,Error] = i3dmgx2_ReceiveDataContinuous(SerialLink,Command,SampleRate,varargin)
...
while Read == 1 && 86400*(now - StartTime) <= 1/SampleRate+.1 %The Read == 1 loop reads the buffer until it finds a correct packet
    ByteNum = 0;
    while ByteNum == 0;
        PossibleRecord = i3dmgx2_ReadNextRecord(SerialLink,Record); %Find a data packet
